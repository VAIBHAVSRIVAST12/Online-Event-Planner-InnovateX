...
public class VendorServiceImpl implements VendorService{
	...
	public double calculatingAmount(int vendorId, String packageType, int planRequestId) throws PlanRequestNotFoundExceptions {
		...
		String[] servicesList = services.split("\\s+");
		List<String> servicesListAsList = Arrays.asList(servicesList);
		double decorationAmount=0;
		double photographyAmount=0;
		double catering_vegAmount=0;
		double catering_nonvegAmount=0;
        if(servicesListAsList.contains("Decorations")){
        	 decorationAmount = packages.getDecoration();
        	
        }
        if(servicesListAsList.contains("Photography")){
        	photographyAmount = packages.getPhotography();
       	
       }
        if(servicesListAsList.contains("Catering")){
       	 catering_vegAmount = packages.getCatering_veg();
       	  catering_nonvegAmount = packages.getCatering_nonveg();
