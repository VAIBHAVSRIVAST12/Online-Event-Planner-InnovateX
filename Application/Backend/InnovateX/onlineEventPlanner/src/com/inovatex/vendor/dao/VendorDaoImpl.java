...
public class VendorDaoImpl implements VendorDao{
	...
	public List<Packages> viewPackages(int vendorId) throws PackageNotFoundExceptions {
		String selectquery="select * from package where vendorid=?";
	...
	public int createQuoation(Quotation quotation) throws QuotationAlreadyExistsException {
		conn = DBConnections.getConnection();
		int rows=0;
		String insertsql = "insert into quotation (QuotationEstimatedAmount,VendorID,UserID,PlanRequestID,packageid,packagetype) values (?,?,?,?,?,?)";
		try {
			preparedStatement = conn.prepareStatement(insertsql);
			...
			preparedStatement.setString(6, quotation.getPackageType());
			rows = preparedStatement.executeUpdate();
        } catch (SQLIntegrityConstraintViolationException e) {
//        	 
	...
	public List<Quotation> viewQuotation(int userId) throws QuotationNotFoundExceptions {
		String selectquery="select * from Quotation where userid=?";
		...
		try {
			...
			{			
				...
				quotation.setPackageType(resultSet.getString("packagetype"));
	...
	public List<Quotation> viewQuotationForVendor(int vendorId) throws QuotationNotFoundExceptions {
		String selectquery="select * from Quotation where vendorId=?";
		List<Quotation> quotationList = new ArrayList<>();
		conn=DBConnections.getConnection();
		try {
			preparedStatement=conn.prepareStatement(selectquery);
